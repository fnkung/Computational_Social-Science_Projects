...
# We talked about how the synthetic control should set treatment = 1 for all states for years that are after their own date adopted -- but this would be the setup for following section on the staggered adoption multisynth, when I ran it with the code below setting up that treatment, it ran it as multisynth
...
-   **Answer**: Ohio and Kentucky were states that instituted the work requirements. Unfortunately, my results did not show a difference between states that instituted vs those that didn't California and Vermont. And the average ATT was zero. However, this was a sample of only 4 states, so this is not sufficient evidence either way.
...
-   **Answer**: For the time cohort, the ATT shows basically no effect, since its so small -0.004 and the STD is also 0.004. However, one issue is that there aren't a lot of pretreatment periods.
